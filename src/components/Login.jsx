...
export default function Login({ onDone }) {
  ...
  const submitSignup = (e) => {
    ...
    try { localStorage.setItem('demoUser', JSON.stringify({ name, email })); } catch {}
  ...
  return (
    <div style={{
      ...
      <form
        ...
        style={{
          width:360, padding:24, borderRadius:16, background:'#fff',
          boxShadow:'0 12px 32px rgba(0,0,0,.08)', display:'grid', gap:12
        }}
      >
        {/* 제목 고정 */}
        ...
        <button
          type="button"
          onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setPw(''); setPw2(''); }}
          style={{
